...
(defn api-key
  ...
  []
  (secrets.core/require-secret!! :open-ai-key))
...
(defn chatgpt
  ...
  (let [url (str (get _config :url "https://api.openai.com/v1") "/chat/completions")
        ...
        response (try (client/post url {:headers headers
                                        :body    body
                                        :as      (if stream? :stream :json)}) (catch Exception e (.printStackTrace e) (println "Error on :" (:url config) " with:" (:model config) "Body is:\n" body) (throw e)))]
    (if stream?
