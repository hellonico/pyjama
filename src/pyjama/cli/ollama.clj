...
(defn -main [& args]
  (let [{:keys [url images model prompt stream chat]} (parse-cli-options args)]
    ...
    #_
    (pyjama.core/ollama url :generate
                        {:model "llama3.2" :prompt "Why is the sky blue?" :options {:seed 123}})

    ; Load a Model
    #_
    (->
      (pyjama.core/ollama url :generate {:model "qwq"} pyjama.core/response)
      (println))

    #_
    (->
    (pyjama.core/ollama url :generate {:stream true :model model :prompt prompt} pyjama.core/response)
    (println))
