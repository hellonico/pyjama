...
(deftest non-streaming-full-json-output
  (->
...
(deftest delete-model
  (->
    (pyjama.core/ollama URL :delete
                        {:model "llama3.2:1b"} identity)
    (println)))

(deftest ps
  (->
    (pyjama.core/ollama URL :ps)
    (println)))

(deftest streaming
  (pyjama.core/ollama URL :generate {:stream true :model model :prompt prompt}))

(deftest llava-and-image-from-base64
  (->
    (pyjama.core/ollama URL
                        :generate
                        {
                         :model  "llava"
...
(deftest llava-and-image-from-file
  (pyjama.core/ollama URL :generate
                      {:model  "llava"
                       :prompt "what is in this picture?"
                       :stream true
